...
try:
    ...
    print("Errore: file 'build/q.csv' non trovato. Esegui prima il programma C++.")
    exit()
...
for i in range(1, 7):
    plt.plot(df["Time"], df[f" q_{i}"], label=f"Componente q{i}")

# Aggiungi etichette, titolo e legenda per rendere il grafico leggibile
plt.title("Evoluzione delle Componenti del Vettore 'q' nel Tempo")
plt.xlabel("Tempo (s)")
