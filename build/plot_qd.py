...
try:
    df = pd.read_csv("qd.csv")
except FileNotFoundError:
    print("Errore: file 'qd.csv' non trovato. Esegui prima il programma C++.")
    exit()
...
for i in range(1, 7):
    plt.plot(df["Time"], df[f" qd_{i}"], label=f"Componente qd{i}")

# Aggiungi etichette, titolo e legenda per rendere il grafico leggibile
plt.title("Evoluzione delle Componenti del Vettore 'qd' nel Tempo")
plt.xlabel("Tempo (s)")
